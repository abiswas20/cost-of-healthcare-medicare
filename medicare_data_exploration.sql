...
-- WE ALSO LOOK AT PROVIDERS IN EACH ZIP CODE
SELECT PROVIDER_ID, COUNT(DISTINCT(PROVIDER_ID)), COUNT(DISTINCT(PROVIDER_NAME)), COUNT(drg_definition), COUNT(DISTINCT(drg_definition)) FROM `bigquery-public-data`.medicare.inpatient_charges_2011 GROUP BY PROVIDER_ID;
